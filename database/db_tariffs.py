...
def add_tariff(
    ...
    with get_db() as conn:
        ...
        logger.info(
            "Добавлен тариф: %s (ID: %s, трафик: %s ГБ, группа: %s, max_ips: %s, billing_type: %s)",
            name, tariff_id, traffic_limit_gb, group_id, max_ips, billing_type,
        )
        return tariff_id
...
def update_tariff(tariff_id: int, **fields) -> bool:
    ...
    if fields.get('billing_type') == 'recurring':
        ...
        fields.setdefault('billing_period_days', fields.get('duration_days') or 0)
