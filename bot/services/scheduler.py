...
async def create_backup_archive() -> Optional[bytes]:
    ...
    Включает:
    - wavemesh_bot.db — база данных бота
        ...
        with zipfile.ZipFile(archive_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
            ...
            if os.path.exists(bot_db_path):
                ...
                fd, temp_bot_db_backup = tempfile.mkstemp(
                    prefix='wavemesh_bot_snapshot_',
                    suffix='.db',
                    dir=BACKUP_DIR,
                )
                os.close(fd)
                snapshot_path = backup_bot_database_to(temp_bot_db_backup)
                zf.write(snapshot_path, 'wavemesh_bot.db')
                logger.info(f"Добавлен в архив: wavemesh_bot.db ({snapshot_path.stat().st_size} байт)")
            else:
...
async def save_local_backup() -> None:
    ...
    try:
        ...
        bot_db_path = os.path.abspath(BOT_DB_PATH)
        if os.path.exists(bot_db_path):
            dest = os.path.join(day_dir, 'wavemesh_bot.db')
            backup_bot_database_to(dest)
            logger.info(f"Локальный бэкап: wavemesh_bot.db ({os.path.getsize(dest)} байт)")
        else:
            logger.warning(f"База данных бота не найдена: {bot_db_path}")
