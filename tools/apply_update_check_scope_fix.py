...
SCHEDULER = Path("bot/services/scheduler.py")
TEST_FILE = Path("tests/test_scheduler_update_notifications.py")
...
def main() -> None:
    ...
    count = source.count(LOCAL_IMPORTS)

    if count not in {0, 1}:
        raise RuntimeError(
            f"unexpected branch-local import count: {count}"
        )

    if count == 1:
        SCHEDULER.write_text(
            source.replace(LOCAL_IMPORTS, "", 1),
            encoding="utf-8",
        )
